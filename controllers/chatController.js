...
class ChatController {
  ...
  async fetchUserFromDB(userId) {
    try {
      ...
      const result = await db.query(
        'SELECT id, name, image, phone, location FROM users WHERE id = $1',
        [userId]
      );
      
      console.log(`📊 User ${userId} query result:`, result.rows);
  ...
  async fetchProductFromDB(productId) {
    try {
      ...
      console.log(`📊 Product ${productId} query result:`, result.rows);
  ...
  async getOrCreateChannel(buyerId, sellerId, productId, additionalData = {}) {
    try {
      ...
      // 1. Fetch user data from database using PostgreSQL syntax
      const buyerResult = await db.query(
        'SELECT id, name, image, location FROM users WHERE id = $1',
        [buyerId]
      );
      ...
      const channel = this.client.channel('commerce', channelId, {
        ...
        members: [String(buyerId), String(sellerId)],
        created_by_id: String(buyerId),
